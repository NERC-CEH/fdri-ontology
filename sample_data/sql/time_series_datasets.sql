...
create table if not exists TIMESERIES_MEASURES as from read_csv('./build/time_series_measures.csv', AUTO_DETECT=true) ;

COPY(
    SELECT * FROM (
    SELECT SITE_INSTRUMENTATION.SITE_ID AS SITE_ID,
        PARAMETERS_INSTRUMENTS.PARAMETER_ID AS PARAMETER_ID,
        TIMESERIESDEF.TIMESERIES_ID AS TIMESERIES_ID,
        TIMESERIESDEF.TIMESERIES_NAME AS TIMESERIES_NAME,
        MIN(START_DATETIME) AS START_DATE,
        MAX(TIMESERIESDEF.PROCESSING_LEVEL) as PROCESSING_LEVEL,
        MAX(TIMESERIESDEF.DURATION) AS PERIOD
    ...
    FROM SITE_INSTRUMENTATION
        ...
        GROUP BY SITE_INSTRUMENTATION.SITE_ID, 
            PARAMETERS_INSTRUMENTS.PARAMETER_ID,
            TIMESERIESDEF.TIMESERIES_ID,
            TIMESERIESDEF.TIMESERIES_NAME
    ) AS A
    LEFT JOIN TIMESERIES_S3 ON A.TIMESERIES_ID = TIMESERIES_S3.TIMESERIES_ID
    LEFT JOIN TIMESERIES_MEASURES ON A.TIMESERIES_ID = TIMESERIES_MEASURES.TIMESERIES_ID
